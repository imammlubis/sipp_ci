...
class DataPerusahaan extends CI_Controller{
    ...
    public function __construct()
    {
        ...
        $this->load->model('CompanyEmailModel');
    }
    function index(){
        check_user_sess();
//        $this->load->view('account/home');
    ...
    public function ajax_list()
    {
        ...
        foreach ($list as $company) {
            ...
            $row[] = '<a class="btn btn-sm btn-primary" href="javascript:void(0)" title="View" onclick="view_email('."'".$company->id."'".')"><i class="glyphicon glyphicon-link"></i> View</a>';
            $data[] = $row;
        }
    ...
    public function ajax_view_email($id)
    {
        $data = $this->CompanyEmailModel->get_by_company_id($id);
        //$data->dob = ($data->dob == '0000-00-00') ? '' : $data->dob; // if 0000-00-00 set tu empty for datepicker compatibility
        echo json_encode($data);
    }
